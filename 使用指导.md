...
8. [常见问题](#常见问题)
9. [附录 A：安装和配置 JACG 分析器](#附录-a安装和配置-jacg-分析器)
...
JACG（java-all-call-graph-server）是一个独立的 MCP Server，为 TIA 提供 Java 方法调用链分析能力。它需要**单独安装**。
...
- **JDK 8** 及以上版本（推荐 JDK 8u462+，部分旧版 JDK 8 有 TLS 兼容问题）
- Gradle（已内置 `gradlew`，无需全局安装）
- Git
...
构建完成后，可执行 JAR 位于 `build/libs/java-all-call-graph-server-*.jar`。
...
# 或直接用 java 启动
java -jar java-all-call-graph-server-*.jar
```

默认监听 **34567** 端口，MCP SSE 端点为 `http://127.0.0.1:34567/mcp/sse`。
...
执行一次含 `.java` 文件的影响分析，观察输出：

```
📦 my-backend  (abc123 → def456)
   🧠 Java 调用链分析 [jacg]: 3 个语义分析项   ← 表示 JACG 已生效
   ⚠️ Java 调用链分析 [jacg]: 不可用，已降级   ← 表示 JACG 未启动或未启用
```
