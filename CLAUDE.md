...
│  └─────────────────────────────────────────────┘     │
│  ┌─────────────────────────────────────────────┐     │
│  │ 模块 3: Analyzer Registry (✅ Phase 5a)       │     │
│  │  ├─ MCP 编织层 — 委托下游分析器                │     │
│  │  ├─ JACG 适配器 (Java 调用链分析)              │     │
│  │  └─ 横向扩展: SQL/Perf/Python/Go 分析器        │     │
│  └─────────────────────────────────────────────┘     │
│  ┌─────────────────────────────────────────────┐     │
│  │ 安全层: Security (✅ 已实现)                  │     │
...
/repo_reset module=订单系统 --label "S26" --since 2026-06-13  # 迭代切换
...
├── paths.ts              # 项目根目录定位 (ESM __dirname 等价)
...
│   ├── handler.ts        # impact_analysis 工具 + 分析器编织逻辑
│   ├── recommendation.ts # test_recommendation 推荐引擎
│   ├── risk-scorer.ts    # risk_assessment 风险评分引擎
│   └── risk-handler.ts   # risk_assessment 工具处理器
├── analyzer-registry/    # 分析器编织层 (Phase 5a)
│   ├── types.ts          # AnalyzerAdapter 接口 + AnalyzerConfig
│   ├── state.ts          # analyzers.conf.json 读写 + 种子文件
│   ├── registry.ts       # 分析器注册表: 加载/匹配/降级
│   └── adapters/
│       └── jacg.ts       # JACG MCP 适配器 (可用性检测 + 降级标记)
...
| **分析器编织层** | `analyzer-registry/*.ts` | Phase 5a: AnalyzerAdapter 接口 + 分析器注册表 + 下游 MCP 委托 | 类型层 + 影响分析层 |
...
```
monitors.conf.json   ←  用户手写（RepoConfig[]）— 仓库 URL / 分支 / 认证 / platform / repoType / module
monitors.json        ←  程序维护（MonitorStateFile）— lastSha / lastCheck / seenShas / snapshots
impact-rules.conf.json ← 用户手写（ImpactRule[]）— 文件→测试映射规则
analyzers.conf.json  ←  用户手写（AnalyzerConfig[]）— 下游分析器注册 + 连接信息
server.conf.json     ←  用户手写（ServerConf）— HTTP 端口 / IP 白名单 / API KEY 哈希
```
...
| Phase 5a | Analyzer Registry — MCP 编织层 + JACG 适配器 | ✅ |
| Phase 5b | JACG 实际调用链分析（Phase 5a 仅可用性检测） | 🔜 |
