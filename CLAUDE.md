...
本项目最终运行在**企业内网**，所有设计决策必须遵循：

1. **简单优先** — 能用函数就不用类，能内联就不抽象，能用配置就不写代码
2. **实用至上** — 只解决当前真实存在的问题，不为"未来可能需要"而设计
3. **可扩展但不预扩展** — 预留扩展点但不预建扩展实现，接口只做一层抽象
4. **开箱即用** — 新机器 clone 后一键启动，零手动配置步骤
5. **免维护** — 不引入需要定期运维的外部依赖（数据库、缓存服务、消息队列）
6. **过度设计零容忍** — 任何超过一层间接调用的设计、仅使用一次的抽象接口、超过 10 个字段未被消费的类型，都必须有充分理由
