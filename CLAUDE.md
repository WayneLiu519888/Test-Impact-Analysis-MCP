...
│  ┌─────────────────────────────────────────────┐     │
│  │ 安全层: Security (✅ 已实现)                  │     │
│  │  ├─ IP 白名单 (精确 IP + CIDR 子网)           │     │
│  │  ├─ API KEY 列表签发 + SHA-256 哈希校验      │     │
│  │  ├─ TIA-init: 客户端自助初始化引导            │     │
...
└── platforms/            # Git 平台适配器层（不感知 MCP 协议，纯数据层）
    ...
    ├── generic.ts        # 通用 REST API（CodeHub/GitLab）+ RSA 签名
    └── local.ts          # 本地 git 命令
```
...
```
src/
├── index.ts              # 入口 + 路由汇总（按模块分发）
├── state.ts              # 共享状态层
├── types.ts              # 共享类型
├── git-monitor/          # 模块 1: Git Monitor
│   ├── tools.ts
│   ├── state.ts
│   └── platforms/
└── impact-analysis/      # 模块 2: Impact Analysis
    ├── tools.ts
    └── ...
```
